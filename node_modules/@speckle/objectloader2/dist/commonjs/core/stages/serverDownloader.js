...
class ServerDownloader {
    ...
    initialize(params) {
        ...
        this.#total = total;
        this.#downloadQueue = new batchingQueue_js_1.default({
  ...
  Chrome's behavior: Chrome generally handles larger data sizes without this specific RangeError. It might become slow or run out of general memory, but not typically with this specific error.
    */
    ...
    async downloadBatch(params) {
        ...
        if (keys.size > 0) {
            throw new Error('Items requested were not downloaded: ' + (0, functions_js_1.take)(keys.values(), 10).join(','));
        }
    ...
    #processLine(line) {
        ...
        throw new errors_js_1.ObjectLoaderRuntimeError('Invalid line format in response: ' + this.decodeChunk(line));
    }
    #processJson(baseId, unparsedBase) {
        ...
        if ((0, functions_js_1.isBase)(base)) {
            return { baseId, base };
        }
        else {
    ...
    #isValidBytes(json) {
        for (const rawEncoding of this.#rawEncodings)
            if ((0, functions_js_1.indexOf)(json, rawEncoding) !== -1) {
                return false;
            }
        return true;
    }
    ...
    async downloadSingle() {
        ...
        if (!this.#isValidString(responseText)) {
            throw new errors_js_1.ObjectLoaderRuntimeError('Invalid base object');
        }
