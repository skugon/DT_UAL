...
class TaskImpl {
    ...
    and(other) {
        return new Task((resolve, reject) => {
            this.#promise.then(matchResult({
                Ok: (_) => {
                    other.#promise.then(matchResult({
                        Ok: resolve,
                        Err: reject,
                    }));
                },
    ...
    andThen(thenFn) {
        return new Task((resolve, reject) => {
            this.#promise.then(matchResult({
                ...
                thenFn(value).#promise.then(matchResult({
                    Ok: resolve,
                    Err: reject,
                })),
                Err: reject,
        ...
        return new Task((resolve, reject) => {
            this.#promise.then(matchResult({
                ...
                Err: (reason) => {
                    ...
                    elseFn(reason).#promise.then(matchResult({
                        Ok: resolve,
                        Err: reject,
                    }));
                },
     ...
     */
    match(matcher) {
        return this.#promise.then(matchResult({
            Ok: matcher.Resolved,
...
export function safelyTryOr(rejection, fn) {
...
export function safelyTryOrElse(onError, fn) {
 ...
 */
export function withRetries(retryable, strategy = (function* () {
    for (let i = 0; i < 3; i++) {
        yield 0;
    }
})()) {
...
/**
  ...
  @param cause The previous cause (often another `Error`) that resulted in
    stopping retries.
 */
export function stopRetrying(message, cause) {
...
export function isRetryFailed(error) {
