...
declare type AnyResult = Result<unknown, unknown>;
declare type SomeResult<T, E> = {
    [IsResult]: [T, E];
};
declare type TypesFor<R extends AnyResult> = R extends SomeResult<infer T, infer E> ? {
    ok: T;
    err: E;
} : never;
declare type OkFor<R extends AnyResult> = TypesFor<R>['ok'];
declare type ErrFor<R extends AnyResult> = TypesFor<R>['err'];
 ...
 */
export declare function andThen<T, E, R extends AnyResult>(thenFn: (t: T) => R, result: Result<T, E>): Result<OkFor<R>, E | ErrFor<R>>;
export declare function andThen<T, E, R extends AnyResult>(thenFn: (t: T) => R): (result: Result<T, E>) => Result<OkFor<R>, E | ErrFor<R>>;
/**
...
/**
  ...
  @returns      The `result` if it is `Ok`, or the `Result` returned by `elseFn`
                if `result` is an `Err.
 */
export declare function orElse<T, E, R extends AnyResult>(elseFn: (err: E) => R, result: Result<T, E>): Result<T | OkFor<R>, ErrFor<R>>;
export declare function orElse<T, E, R extends AnyResult>(elseFn: (err: E) => R): (result: Result<T, E>) => Result<T | OkFor<R>, ErrFor<R>>;
/**
 ...
 */
export declare function ap<A, B, E>(resultFn: Result<(a: A) => B, E>, result: Result<A, E>): Result<B, E>;
export declare function ap<A, B, E>(resultFn: Result<(a: A) => B, E>): (result: Result<A, E>) => Result<B, E>;
/**
...
*/
export declare function safe<F extends AnyFunction, P extends Parameters<F>, R extends ReturnType<F>>(fn: F): (...params: P) => Result<R, unknown>;
/**
  Transform a function which may throw an error into one with an identical call
  signature except that it will return a {@linkcode} instead of throwing an
  error.
  ...
  @param handleErr A function to use to transform an unknown error into a known
    error type.
*/
export declare function safe<F extends AnyFunction, P extends Parameters<F>, R extends ReturnType<F>, E>(fn: F, handleErr: (error: unknown) => E): (...params: P) => Result<R, E>;
/**
